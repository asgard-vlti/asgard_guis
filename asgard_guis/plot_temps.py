...
class TempPlotWidget(QtWidgets.QWidget):
    def __init__(self, interval, lookback, log_path, window, parent=None):
        ...
        gs = self.figure.add_gridspec(
            n_plts, 1, height_ratios=[2, 1, 1, 1, 1], hspace=0.1
        )
        # Create all axes, sharing x only with the last subplot
        self.axes = [self.figure.add_subplot(gs[i, 0]) for i in range(n_plts)]
    ...
    def update_plot(self):
        ...
        if self.lookback is not None:
            ...
            cutoff = times[-1] - timedelta(minutes=self.lookback)
        ...
        if len(times) > 1:
            ...
            window_samples = max(1, int(round(self.window / median_interval)))
        else:
            window_samples = 1
...
def get_log_by_index(idx):
    log_dir = os.path.expanduser(os.path.join("~", "logs", "templogs"))
    files = sorted(
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot temperature logs from temperature_watchdog."
    )
    parser.add_argument(
        "logfile",
        nargs="?",
